...
## 2026-09-11 添加栏智能联想需求审阅（待实施）

已对照当前添加流程、代码解析、期货目录和快照封套，修订 `SPEC.md` §3.2/§3.2.1 与
[`详细需求`](docs/requirements-stock-search-suggest.md)。补齐批量/候选提交优先级、输入法与组件生命周期、
曾用名来源及维护、截短名恢复、状态时效、稳定排序、性能预算及 S01–S17 验收矩阵。
本轮仅交付文档；数据源真实验证、字典构建、功能代码和功能验收均待实施，不代表已上线联想搜索。
