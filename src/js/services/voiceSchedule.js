...
export function decideVoiceSchedule({ codes, settings, now, tradingDates = [], previous = null }) {
  ...
  const hasExtendedHoursAssets = list.some((c) => inferAssetType(c) !== ASSET_TYPES.STOCK_CN);
  ...
  // Stock-only automatic close remains a persisted off switch. Extended-hours assets
  // (futures, HK, US) retain the user's armed preference across market gaps.
  if (cfg.enabled && enabled && !hasExtendedHoursAssets && session === 'after-close' && cfg.autoStopAfterClose) enabled = false;
