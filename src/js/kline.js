...
export function getPriceLimit(code, name) {
  ...
  // 去掉 sh/sz/bj 前缀后看前缀号
  const numeric = code.replace(/^(sh|sz|bj)/i, '');
  ...
  // 北交所股票代码以 8/43/92 开头（即便无 bj 前缀亦为 30%）
  if (numeric.startsWith('8') || numeric.startsWith('43') || numeric.startsWith('92')) return 30;
  ...
  if (period === '1d' && isAStock) {
    ...
    if (last.preview) {
      ...
      if (hasOfficialTradeEvidence) {
        ...
        // Collapse prices to current preview price and keep volume/amount strictly at 0.
        updated.open = price;
        updated.high = price;
        updated.low = price;
        updated.close = price;
