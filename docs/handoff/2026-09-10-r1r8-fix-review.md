...
> [!NOTE]
> **历史基线标注**：本文审查的是 `33a742d`，其中 R1–R8 只被**部分修复**；本文列出的 M1–M6、m1–m3 缺口其后已全部闭环（`fa24120` 语音队列归属、`c01b3da` 播报确认后再写去重基线、`19c2500` 实时行情合并证据、`f9055e1` 报价封套与过期可见性，以及 m1/m2/m3 收尾）。后续读者请以 `docs/handoff/2026-09-10-m1m6-fix-verification.md` 与 `STATUS.md` 为准；本文的「实测结论」列描述的是 `33a742d` 当时的状态，不是当前 `main` 的状态。配套脚本 `docs/handoff/2026-09-10-review-repro.mjs` 默认校验期望的修复行为（`--expect=fixed`），并保留 `--expect=broken` 供历史对照。
