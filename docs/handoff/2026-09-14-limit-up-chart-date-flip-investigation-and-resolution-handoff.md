...
- **改动方案**：
  实时报价的日期兜底必须优先取当前交易日（从 `../tradeCalendar.js` 导入 [`resolveStockChartDate`](file:///d:/AiPrograms/project1/market-voice-alert/src/js/tradeCalendar.js#L118)），彻底移除对 `inst.selectedTradeDate` 的回退依赖。
- **代码对比**：
  ```javascript
