...
* **分时走势与历史 K 线（Eastmoney）**：
  ...
  * **按交易所精准分发 secid 市场号（彻底纠偏与真实市场号对齐）**：
    ...
    * **恒指期货特别说明**：东财期货市场号（m:100–124）内未收录恒指期货连续合约（`m:100` 实为国际指数现货市场，含 `100.HSI` 恒生现货指数但非期货）。因此 `GL_HSI` 明确确立为**新浪 `hf_HSI` 单源模式**（仅提供实时报价，不提供东财分时/K线），并在展示层和调度层予以显式降级标注。

### 2.2 港股（HK Stocks）实测
* **实时报价**：腾讯 `https://qt.gtimg.cn/q=r_hk00700`（五位代码，实测 78 字段，取值条件：盘中与盘后一致），时间格式为 `YYYY/MM/DD HH:mm:ss`。
...
### 2.3 美股（US Stocks）实测
* **实时报价**：腾讯 `https://qt.gtimg.cn/q=usAAPL`（实测 71 字段，取值条件：非交易时段实测；开盘盘中字段数可能有增补），常规收盘价位于 f3，时间格式为 `YYYY-MM-DD HH:mm:ss`。
...
| `GL_HSI` | 恒生指数期货 | `HKD` | `hf_HSI` | `null` | HKFE | **仅实时报价**（东财无期指合约，明确单源） |

* **真实有效的解析层隔离与冲突防护断言**：
  ```js
  // 单元测试中验证解析层正交性，确保国内代码与外盘代码分派完全正确且互不污染
  import { parseFutureInput, PRODUCT_MAP } from '../src/js/futures/contractCatalog.js';
  ...
  // 2. 验证外盘命名空间 GL_SI0 严格解析为国际期货（COMEX 白银）
...
| **客户端** | `src/js/kline.js:326-345` 与消费方 `:345, 360-367` | 默认回落 10% 涨跌停判定 | **双端改造**：`getPriceLimit` 非 A 股返回 `null`；消费方 (`kline.js:345` / `classifyKlineBar`) 增加 `limit === null` 短路保护，不再经 `Number(limit) \|\| 10` 回落，彻底消除非 A 股涨跌停假标记 |
...
2. **新浪备源降级与单源品种（GL_HSI）特别契约**：
   * 当切至新浪 `hf_` 备用源时，显式使用新浪字段 7（昨结）重校基准；
   * 对东财无期指合约的 `GL_HSI`（恒指期货），明确声明为**纯新浪单源驱动**（仅实时卡片展示，图表区显示“该品种暂无历史分时，仅提供实时报价监控”），消除图表数据为空时的异常报错。
...
2. **新资产功能验收断言**：
   * **secid 有效性**：Phase 1 中具备东财源的 9 个品种均能成功拉取 `trends2` 与 `kline`，单源品种 `GL_HSI` 具备优雅降级断言；
