...
* **分时走势与历史 K 线（Eastmoney）**：
  * **主机轮转架构与约束**：单主机 `push2.eastmoney.com` 对 `trends2` 存在高频 502，且 `push2his` 族主机在部分受限网络环境下存在 `UND_ERR_SOCKET` 连接重置问题（见 `server/marketData.js:14-16` 与 `server/klineService.js:91-95` 探针记录）。经实测，`push2delay.eastmoney.com` 稳定可达且能完整拉取 `m:134`（`trends2` 与 `kline` 938 根）及 `m:100-104` 全品种；因此必须对现有 `EASTMONEY_TRENDS_HOSTS`（`server/marketData.js:18-22`）与 `klineService.js:95` 进行扩展改造，**将 `push2delay.eastmoney.com` 正式追加至轮转与兜底清单中**，并对 `qt/stock/get` 增加 `push2delay` 兜底，确保在不同网络环境下东财数据稳定可达。
  * **按交易所精准分发 secid 市场号（彻底纠偏与真实市场号对齐）**：
    ...
    * **市场号与主机说明**：`m:100` 实为国际指数现货市场（含现货 `100.HSI` 与 `100.XIN9`，非期货合约）；真实恒生指数期货位于 `m:134`；实测在各类网络环境下 `push2delay.eastmoney.com` 均能稳定可达并拉取 `m:134` 与 `m:100-104` 的 `trends2` 与 `kline` 数据，确立 `push2delay` 为核心必备轮转源。
...
| 内部统一代码 | 品种名称 | 计价货币 | 新浪报价符号 | 东财 secid | 交易场所 | 源间缩放 (东财:新浪) | 走势图可用性 |
|---|---|---|---|---|---|---|---|
| `GL_CL0` | 纽约原油 | `USD` | `hf_CL` | `102.CL00Y` | NYMEX | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_GC0` | 纽约黄金 | `USD` | `hf_GC` | `101.GC00Y` | COMEX | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_SI0` | 纽约白银 | `USD` | `hf_SI` | `101.SI00Y` | COMEX | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线（隔离国内工业硅） |
| `GL_HG0` | 纽约美铜 | `USD` | `hf_HG` | `101.HG00Y` | COMEX | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_NG0` | 天然气 | `USD` | `hf_NG` | `102.NG00Y` | NYMEX | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_NQ0` | 纳斯达克期货 | `USD` | `hf_NQ` | `103.NQ00Y` | CME | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_ES0` | 标普500期货 | `USD` | `hf_ES` | `103.ES00Y` | CME | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_YM0` | 道琼斯期货 | `USD` | `hf_YM` | `103.YM00Y` | CME | 1.0 (1:1 直接对齐) | 实时报价 + 分时 + K线 |
| `GL_A50` | 富时中国A50 | `USD` | `hf_CHA50CFD` | `104.CN00Y` | SGX | **0.1** (SGX 0.1点单位，东财÷10 对齐新浪点数) | 实时报价 + 分时 + K线（实测 m:104） |
| `GL_HSI` | 恒生指数期货 | `HKD` | `hf_HSI` | `134.HSI_M` | HKFE | 1.0 (1:1 精确对齐 24676) | 实时报价 + 分时 + K线（东财 m:134 主力合约，新浪 hf_HSI 作备源） |
...
* **真实有效的解析层隔离与冲突防护断言**：
  ...
  > assert.equal(inferAssetType('unknown_foo'), 'stock_cn'); // 兜底防崩
  >
...
| **客户端** | `src/js/parser.js:11-25` (`normalizeCode`) | 仅识别 `sh/sz/bj` | 扩展并新增导出 `inferAssetType(code): string`（契约值域严格对照 `ASSET_TYPES`：① `GL_` 前缀 或 `^hf_[A-Za-z0-9_]+# 国际期货与国际股票接入可行性调研与架构设计方案 (Handoff v3 - 闭环审查修订单)
...
| **服务端** | `server/utils.js:76-86` (`normalizeCodeParam`) | 仅匹配 `^(sh\|sz\|bj)\d{6}$` 或 6 位纯数字，其余返空 | 支持 `hk\d{5}`、`us[A-Za-z]+`、`GL_[A-Z0-9]+`、`hf_[A-Za-z0-9_]+` |
| **服务端** | `server/marketData.js:18-22, 160, 194` | `EASTMONEY_TRENDS_HOSTS` 仅含 `push2his` 族，且代码截取 slice(2) | **主机扩展与资产感知**：将 `push2delay.eastmoney.com` 追加进轮转与兜底清单；按资产类型分发各市场东财 secid 与 Aktools 参数 |
（新浪外盘备源符号） → `'futures_global'`；② `^(?:sh\|sz\|bj)\d{6}$ 或 6位纯数字` → `'stock_cn'`；③ `^(?:hk\|r_hk)\d{5}# 国际期货与国际股票接入可行性调研与架构设计方案 (Handoff v3 - 闭环审查修订单)
...
| **服务端** | `server/utils.js:76-86` (`normalizeCodeParam`) | 仅匹配 `^(sh\|sz\|bj)\d{6}$` 或 6 位纯数字，其余返空 | 支持 `hk\d{5}`、`us[A-Za-z]+`、`GL_[A-Z0-9]+`、`hf_[A-Za-z0-9_]+` |
| **服务端** | `server/marketData.js:18-22, 160, 194` | `EASTMONEY_TRENDS_HOSTS` 仅含 `push2his` 族，且代码截取 slice(2) | **主机扩展与资产感知**：将 `push2delay.eastmoney.com` 追加进轮转与兜底清单；按资产类型分发各市场东财 secid 与 Aktools 参数 |
 → `'stock_hk'`（支持腾讯 `r_` 自动归一化）；④ `^(?:us)?[A-Za-z]+# 国际期货与国际股票接入可行性调研与架构设计方案 (Handoff v3 - 闭环审查修订单)
...
| **服务端** | `server/utils.js:76-86` (`normalizeCodeParam`) | 仅匹配 `^(sh\|sz\|bj)\d{6}$` 或 6 位纯数字，其余返空 | 支持 `hk\d{5}`、`us[A-Za-z]+`、`GL_[A-Z0-9]+`、`hf_[A-Za-z0-9_]+` |
| **服务端** | `server/marketData.js:18-22, 160, 194` | `EASTMONEY_TRENDS_HOSTS` 仅含 `push2his` 族，且代码截取 slice(2) | **主机扩展与资产感知**：将 `push2delay.eastmoney.com` 追加进轮转与兜底清单；按资产类型分发各市场东财 secid 与 Aktools 参数 |
（且非国内期货） → `'stock_us'`；⑤ `isFutureCode(code)===true` → `'futures_cn'`；⑥ 其余未知代码统一兜底回退 `'stock_cn'`），同时改造 `normalizeCode` 统一输出规范化资产对象 `{ code, type, market }` |
...
2. **新浪备源切换与昨结基准对齐机制**：
   ...
   * 当切至新浪 `hf_` 备用源时，显式使用新浪字段 7（昨结）重校基准。
     ...
     * **SGX 0.1点单位归一化**：东财 `104.CN00Y`（SGX 市场 104）按 0.1 点单位报价（数值为新浪的 10 倍，实测东财 `f60=142710`、`f43=143980` 对新浪昨结 `14271.000`、现价 `14400.000`）；备源切换时必须应用 `×0.1` 缩放归一化（`142710 × 0.1 = 14271.000`），以确保第一源与备源切换时价格基准精确无缝对接。
...
2. **新资产功能验收断言**：
   ...
   * **防误路由与备源断言**：断言 `parseFutureInput('SI0')` 保持国内工业硅，而 `GL_SI0` / `hf_SI` / `hf_HSI` / `hf_CL` 经 `inferAssetType` 均准确解析为 `futures_global`，送入会话调度器时绝不误判为 A 股策略；
   * **备源切换锚点缩放断言**：断言 `GL_A50` 在东财与新浪备源切换时应用 0.1 缩放系数，两源归一化昨结误差为 0（`142710 × 0.1 == 14271.0`）；
