...
| 日期 | 文档 | 说明 |
| :--- | :--- | :--- |
| 2026-09-16 | [`2026-09-16-chart-date-format-and-volume-color-handoff.md`](2026-09-16-chart-date-format-and-volume-color-handoff.md) | **缺陷修复**：图表时间轴中国习惯年-月-日格式化与成交量红绿规则修复。① 注册 `zh-CN` / `yyyy-MM-dd` 本土化配置，日周月 K 线关闭 `timeVisible` 消除无意义的 `00:00`，十字线浮层与时间轴均输出中国习惯 `YYYY-MM-DD`（分钟线输出 `YYYY-MM-DD HH:mm`）；② 修复高开低走假阴线收涨（如新农开发 2026-09-11 见顶日收涨 +0.83%）与一字涨停在原 `close > open` 逻辑下被误染为绿柱缺陷，导出 `isVolumeBarUp(it, prevClose)`，综合判定阳线与昨收收涨，并补全实时行情 Tick 的 `prevClose` 上下文。门禁 `npm test` 841/841 + `npx playwright test e2e/chart.spec.js` 12/12 + `npm run lint` 0 + `npm run build` 成功 |
