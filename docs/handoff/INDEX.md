...
> - 最新方案（国际品种）：[`2026-09-16-global-market-feasibility-and-architecture-handoff.md`](2026-09-16-global-market-feasibility-and-architecture-handoff.md)（国际期货与国际股票[港美股]接入可行性调研与架构设计方案：公网免费低延迟数据源实测打通；资产领域模型扩展；策略模式解耦独立交易会话与冬夏令时；分阶段落地规划）
...
| 日期 | 文档 | 说明 |
| :--- | :--- | :--- |
| 2026-09-16 | [`2026-09-16-global-market-feasibility-and-architecture-handoff.md`](2026-09-16-global-market-feasibility-and-architecture-handoff.md) | **架构方案与可行性调研**：国际期货（外盘期货）与国际股票（港美股）接入可行性与架构设计交接。公网低延迟数据源（Sina `hf_` / Tencent `us` `hk` / Eastmoney `102` `105` `116`）现场实测全量打通；资产领域模型扩展（`stock_hk`/`stock_us`/`futures_global`）；多时区交易会话解耦（策略模式防内盘回归）；分时图与TTS货币适配；分阶段演进路线（Phase 1 外盘期货 -> Phase 2 港股 -> Phase 3 美股） |
