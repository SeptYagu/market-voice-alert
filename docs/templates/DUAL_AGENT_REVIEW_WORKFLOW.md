...
- **目标目录**：仓库根目录 `{WORKSPACE_PATH}`。
- **派发方式**：调用 `workbuddy-bridge` 技能（或 WorkBuddy CLI 工具），以非阻塞后台任务派发，主智能体挂起等待回传。
...
【目标工作区】
当前工作目录为：{WORKSPACE_PATH}

【审查任务】
Antigravity 刚刚完成了任务「{TASK_DESCRIPTION}」的代码提交并已推送远端。请执行 `git pull --ff-only` 拉取最新代码，运行基线测试（`{LINT_CMD}` 与 `{TEST_CMD}`），并对最新改动进行严格、独立的质量审查与缺陷核验：
