...
import { limitUpChartMgr, _internal } from '../src/js/app.js';
...
QUnit.module('limitUpChartFixes - 图表历史日期修复与调度解耦测试矩阵', (hooks) => {
  ...
  QUnit.test('用例 5: resolveLiveFallbackDate 隔离保护期货标的交易日，并在盘前正确锚定至前一交易日', (t) => {
    const tradingDates = ['2026-09-10', '2026-09-11', '2026-09-14'];
  ...
  QUnit.test('用例 7: applyDataRefreshSchedule 在涨停看板下保持行情轮询存活 (pollTimerAlive: true)', (t) => {
    ...
    try {
      ...
      t.equal(info.pollTimerAlive, true, '涨停看板挂载后 pollTimerAlive 恒为 true');
