...
export function parseFutureInput(input) {
  ...
  // 移除旧格式前缀 nf_ 或 NF_ 或 nf
  if (clean.startsWith('NF_')) clean = clean.slice(3);
  else if (clean.startsWith('NF') && clean.length > 3 && /\d/.test(clean[2])) clean = clean.slice(2);

  // 中文别名检测，如 "螺纹主连", "白糖主力", "沪铜主连"
  if (clean.endsWith('主连') || clean.endsWith('主力')) {
    const namePrefix = clean.slice(0, -2);
    for (const [prod, info] of Object.entries(PRODUCT_MAP)) {
      if (
        info.name === namePrefix ||
        info.name.startsWith(namePrefix) ||
        namePrefix.startsWith(info.name) ||
        prod === namePrefix
      ) {
